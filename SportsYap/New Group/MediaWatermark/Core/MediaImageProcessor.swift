...
extension MediaProcessor {
    func processImageWithElements(item: SportsYapMediaItem, completion: @escaping ProcessCompletionHandler) {
        if item.filter != nil {
            ...
            filterProcessor.processImage(image: item.sourceImage.fixedOrientation(), completion: { [weak self] (success, finished, image, error) in
                if error != nil {
                    ...
                    let updatedMediaItem = SportsYapMediaItem(image: image!)
    ...
    func processItemAfterFiltering(item: SportsYapMediaItem, completion: @escaping ProcessCompletionHandler) {
