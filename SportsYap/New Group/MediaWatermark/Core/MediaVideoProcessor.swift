...
extension MediaProcessor {
    func processVideoWithElements(item: MediaItem, completion: @escaping ProcessCompletionHandler) {
        ...
        print("the size of the video \(sizeOfVideo)")
    }
    ...
    private func processAndAddElements(item: MediaItem, layer: CALayer) {
        for element in item.mediaElements {
            ...
            if element.type == .view {
                ...
                //elementLayer.backgroundColor = UIColor.clear.cgColor
            ...
            print("element frame")
            print(element.frame)
            print(element.frame.origin.y)
            print(element.frame.maxY)
            print(item.size)
            
            //let rect = CGRect(x: element.frame.origin.x, y: item.size.height - element.frame.maxY, width: element.frame.width - 9, height: element.frame.height)
            
            //let rect = CGRect(x: element.frame.origin.x, y: item.size.height - element.frame.maxY, width: element.frame.width, height: element.frame.height)
            ...
            elementLayer.frame = rect //element.frame
